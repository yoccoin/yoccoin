...
// Copyright (c) 2011-2012 Yocoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #YocoinTEST0\r");
            Send(hSocket, "WHO #YocoinTEST0\r");
        } else {
            // randomly join #Yocoin00-#Yocoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Yocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Yocoin%02d\r", channel_number).c_str());
        }
